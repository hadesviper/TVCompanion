...
import com.prtd.serial.data.remote.dto.getMoviesPopular
...
class PopularMoviesUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<Resources<MoviesPopular>> = flow {
        try {
            emit(Resources.Loading())
            val data =restApiRepo.getPopularMovies()
            emit(Resources.Success(data.getMoviesPopular()))
        }
