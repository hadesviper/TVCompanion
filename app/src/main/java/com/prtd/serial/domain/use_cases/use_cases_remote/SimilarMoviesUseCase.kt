...
class SimilarMoviesUseCase @Inject constructor(
    ...
    operator fun invoke(id: Int, page: Int): Flow<Resources<MoviesResult>> = flow {
        try {
            emit(Resources.Loading())
            val data = restApiRepo.getSimilarMovies(id, page)
            emit(Resources.Success(data.toMovieResults()))
